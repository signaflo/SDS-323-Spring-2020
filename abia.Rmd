...
ggplot(data = cancelled, aes(x = UniqueCarrier)) + 
  geom_bar(position = "fill", 
           aes(y = (..count..)/sum(..count..), fill = CancellationCode))
```
...
```{r}
ggplot(abia %>% filter(!is.na(DepTime)), aes(x = DayOfWeek, y = CancellationCode), fill = "color") + 
  geom_bar(stat= "identity")
```
## debug cancelled by month and destination

ggplot(cancelled, aes(x = Month, y = Cancelled, fill = Dest)) + geom_bar(stat = "identity")



# cancelled out of all flights
carrtotals = abia %>% group_by(Dest) %>%summarize(total.count=n())

cantotals=abia %>% group_by(Dest) %>% summarize(cantot=sum(Cancelled==1))

canprop=cbind(cantotals[1], round(cantotals[-1]/cartotals[-1], 4))

canprop

# check with destination as x and y as cancelledprop?


# redo attempt
ggplot(cantotals, aes(x = Month, y = Cancelled, fill = Dest)) + geom_bar(position = "fill", stat = "identity") +
...
```{r}
# droplevels()
# try with proportion - good
ggplot(data = most_visited, 
  aes(x = Month, y = (Cancelled==1)/(sum(Cancelled)), fill = Dest)) + 
  geom_bar(stat = "identity") +
