...
output_values_2 = foreach(k = k_to_try, .combine = rbind) %do% {
  out2 = do(reps) * {
    ...
    knn_model_predictions_2 = knn_model_2$pred
     ...
     # out of sample confusion matrix
    viral_prediction_2 = ifelse(knn_model_predictions_2 > 1400, 1, 0)
    actual_viral <- ifelse(y_test > 1400, 1, 0)
    
    confusion_matrix2 = table(y = actual_viral, yhat = viral_prediction_2)
    output2 <- c(output2, calc_class_err(y_test2, knn_model_predictions_2, n_test))
...
rmse_avg2 <- output_values_2[,"rmse"]
output_values_2 <- output_values_2[,2:5]
best_rmse_k2 = k_to_try[which.min(rmse_avg2)]
best_rmse_k2

rmse_best_k2 = min(rmse_avg2)
rmse_best_k2
...
```{r  }
confusion_out_2 = table(knn.pred = knn_model_predictions_2, Actual = D_test2$viral) 
