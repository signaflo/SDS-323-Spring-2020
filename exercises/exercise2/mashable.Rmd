...
k_grid <- exp(seq(log(1), log(900), length=30)) %>% round %>% unique
k_grid <- k_grid[k_grid != 2]
k_grid
```
Iterating across a range of K to find the best out-of-sample accuracy rate, we find that the best K value is when K = [ insert best K here ].
```{r echo = FALSE}
# create a confusion matrix for multiple K values
## debug FIXME need to find the optimal K!!!!!!!!!!!!!!!!
set.seed(1)
confusion_valse = 
...
best_k
## debug find the K associated with this!!! says lowest K but seems wrong.
...
set.seed(1)
knn3 = knn.reg(train = X_train, 
               test = X_test, y = y_train, k=24)  
## debug need to place best k above once correct
ypred_knn3 <- knn3$pred
